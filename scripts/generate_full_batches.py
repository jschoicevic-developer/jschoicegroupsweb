...
print(f"Loaded {len(records)} records from ndis_records.json")

# Split into batches of 50 (larger batches for speed)
batch_size = 50
batches = [records[i:i + batch_size] for i in range(0, len(records), batch_size)]

print(f"Splitting into {len(batches)} batches of size {batch_size}")
...
for batch_num, batch in enumerate(batches, 1):
    ...
    filename = f'scripts/full_batch_{batch_num:02d}.sql'
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(sql)
    
    print(f"Generated {filename} ({len(batch)} records)")

print("Done. Generated full import batches.")
