...
import { AREAS_SERVED } from "@/data/areas-served";

interface AreasWeServeSectionProps {
    /** The only field that changes per page — everything else is shared/fixed. */
    description: string;
}

/**
 * Reusable "Areas We Serve Across Melbourne" section.
 * Layout, heading and location list are fixed and shared across every page
 * that renders it (matches the original home page design 1:1).
 */
const AreasWeServeSection = ({ description }: AreasWeServeSectionProps) => {
    return (
